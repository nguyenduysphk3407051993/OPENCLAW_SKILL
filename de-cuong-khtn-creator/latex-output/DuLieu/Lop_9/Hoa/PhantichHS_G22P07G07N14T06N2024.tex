...
%%%%==============Vidu5==============%%%
\begin{vd}[Phân tích mối liên hệ giữa số mol $CO_2$, số mol $H_2O$ của sản phẩm đốt cháy]
...
\begin{vd}[Phân tích các mối liên hệ trong phân tử chất béo]
	Hỗn hợp E gồm các chất béo và axit béo tự do (trong đó oxi chiếm 10,88\% theo khối lượng). Xà phòng hóa hoàn toàn $m$ gam $E$ bằng dung dịch $\mathrm{NaOH}$ dư đun nóng, sau phản ứng thu được dung dịch chứa 103,3 gam hỗn hợp các muối $C_{17} H_{35} \mathrm{COONa}, C_{17} H_{33} \mathrm{COONa}, C_{17} H_{31} \mathrm{COONa}$ và 10,12 gam glixerol. Mặt khác, $m$ gam $E$ phản ứng tối đa với $x$ gam $\mathrm{Br}_2$ trong dung dịch. Tính giá trị của $x$?
	\loigiai{
				\Nhanmanh[\faComments]{Phân tích:} Các muối đều có 18C. Đây là cơ sở để ta đặt công thức trung bình của hỗn hợp muối thu được là $C_{17} H_{35-2k} COONa$.
				Muối $C_{17} H_{35-2k} COONa$ có khối lượng mol $M=306-2k$
				 \\
				 $\Rightarrow m_{\text{muối}}=306n_{\text{muối}}-2n_{C=C}$.
				 \\ 
				 Như vậy để tìm số mol $\mathrm{Br}_2$ ta chỉ cần biết số mol muối (bằng số mol nhóm-COO-)
				 \begin{center}{\indam{Lời giải:}}\end{center}
				Tính số $\mathrm{mol}_{C_3 H_5(OH)_3}=\dfrac{10,12}{92}=0,11$ (mol)
				\\
				Phương trình phản ứng:
				\[\begin{array}{ccccccccc}
						& (\mathrm{RCOO})_3 \mathrm{C}_3 \mathrm{H}_5&+&3 \mathrm{NaOH} &\xrightarrow& 3\mathrm{RCOONa}&+&\mathrm{C}_3\mathrm{H}_5(\mathrm{OH})_3& \\
						& 0,11 &&& &0,33&  &\xleftarrow[][][0.5][][>=stealth]0{,}11 &(mol) \\
						& \mathrm{R}^{\prime}\mathrm{COOH}&+&\mathrm{NaOH}&\xrightarrow& \mathrm{R} ' \mathrm{COONa}&+&\mathrm{H}_2\mathrm{O}& \\
						&\mathrm{a} &&&&\mathrm{a}&&& (mol)\\
				\end{array}\]
				...
				\end{gathered}\]
		$n_{\mathrm{Br}_2}=n_{C=C}=0{,}37$ $\mathrm{mol}$ $\Rightarrow x=0{,}37\cdot160=59{,}2$ (gam).
	}
\end{vd}
%%%%==============HetVidu6==============%%%
...
\begin{vd}[Câu hỗn hợp kim loại kiềm,kiềm thổ và oxit của chúng]
	...
	\loigiai{
		\Nhanmanh[\faComments]{Phân tích:} Đây là bài toán có nhiều cách giải (BT mol kết hợp BTKL, phương pháp quy đổi, phương pháp sư dụng bảo toàn electron\ldots các phương pháp này cũng khá hiệu quả. Tuy nhiên, để thấy vẻ đẹp về các mối liên hệ trong phương trình hóa học ta hãy xem xét bài toán qua phương pháp sư dụng công thúc đại diện kết hợp phân tích hệ số.
...
%%%%%==============HetVidu7==============%%%
%%%%%==============Vidu8==============%%%
%\begin{vd}[Phân tích tăng giảm số mol trong phản ứng toàn chất khí]
...
%				Phân tích hệ số phản ứng: $2CH_4 \xrightarrow[$t^\circ$] C_2 H_2+3H_2$ thấy $\Delta$ hệ số $=1+3-2=2=$ hệ số $CH_4$. Như vậy trong phản ứng này số mol khi tăng lên đúng bằng số mol khí $CH_4$ phản ứng.
...
\begin{vd}[Phân tích tăng giảm số mol trong phản ứng toàn chất khí]
	...
	Tính Hiệu suất phản ứng nhiệt phân metan và tỉ khối của $X$ so với He.
	\loigiai{
		\Nhanmanh[\faComments]{Phân tích:} Đây là một bài tập không khó nhưng khá hay. Dễ thấy hỗn hợp truớc và sau phản ứng đều giữ nguyên các nguyên tố ban đầu nên không lượng không đổi ($\left.m_X=m_{CH_4}=8\mathrm{gam}\right)$.
		\\
		Phân tích hệ số phản ứng: $2CH_4 \xrightarrow[$t^\circ$] C_2 H_2+3H_2$ thấy $\Delta$ hệ số $=1+3-2=2=$ hệ số $CH_4$.
		\\
		Như vậy trong phản ứng này số mol khi tăng lên đúng bằng số mol khí $CH_4$ phản ứng.
		\\
		Tính số mol khí $X$ trong bình kín 7 lít ($0^{\circ} C$, 3amt) thì số mol gấp 3 lần so với $\text{ĐKTC}\left(0^{\circ} C\right.$; 1atm) do số mol tỉ lệ thuận với áp suất (khi thể tích và nhiệt độ không đổi) $\Rightarrow n_X=3\cdot(7: 22{,}4)=0{,}9375$ $\mathrm{mol}$
		\\
		$n_{CH_4}=\dfrac{8}{16}=0,5$ $\mathrm{mol}$; $n_X=\dfrac{3.7}{22,4}=0,9375$ $\mathrm{mol}$
		\\
		Gọi h là hiệu suất phản ứng nhiệt phân $CH_4\left(\mathrm{h}=\dfrac{H\%}{100}\right)$
		$\Rightarrow n_{CH_4}(\text{pur})=0,5 \cdot \dfrac{H}{100}=0,5 \mathrm{h}$ $(\mathrm{mol})$
		\[\begin{array}{ccccccc}
		&2CH_4 &\xrightarrow[$t^\circ$][][][0.5]& C_2 H_2&+&3H_2&\\
		&0,5\mathrm{h}&&\xrightarrow[][][0.5]0,25\mathrm{h}&&\xrightarrow[][][0.5]0,75\mathrm{h})& (mol)
		\end{array}\]
		$\Rightarrow 0{,}5+0{,}5\mathrm{h}=0{,}9375\Rightarrow h=0{,}875$
		\\
		Vậy hiệu suất phản ứng: $H\%=h.100\%=0{,}875.100\%=87{,}5\%$
...
%%%%==============HetVidu8==============%%%
%%%%==============Vidu9==============%%%
\begin{vd}[Phân tích tăng giảm khối lượng trong phản ứng kim loại tác dụng với muối]
	...
	\loigiai{
			\Nhanmanh[\faComments]{Phân tích:} Mấu chốt bài toán ở các đặc điểm sau:
			
			-- Xem xét TGKL kim loại ta thấy: $\mathrm{Zn} \xrightarrow \mathrm{Cu}$ (khối luợng giảm); Fe $\xrightarrow \mathrm{Cu}$ (khối luợng tăng).
			
			Nếu Fe chưa phản ứng thì khối lượng KL giảm. Theo đề khối lượng KL lại tăng $\Rightarrow$ chúng tỏ Fe đã phản ứng, Zn đã hết.
			
			Mặt khác: Nếu chuyển toàn bộ E thành oxit thì $\dfrac{5,96.81}{65}=7,427(\mathrm{g}) < m_{\text{oxit}} < \dfrac{5,96.160}{112}=8,514\mathrm{gam}$. Điều này chúng tỏ kim loại Fe còn dur. Nếu không khai thác được dũ kiện này thì phải biện luận trường hợp.
			
			Kỹ thuật phân tích tăng giảm khối lượng trong toán KL tác dụng muối có lẽ không xa lạ với giáo viên chúng ta. Tuy nhiên, xử lý nó như thế nào thì còn tùy thuộc vào phong cách riêng của mỗi người.
			\begin{center}{\indam{Lời giải:}}\end{center}
...
%%%%==============Vidu10==============%%%
\begin{vd}[Hỗn hợp khí có số mol khi không đổi trong phản ứng]
	Hỗn hợp $X$ gồm $CH_4$, $H_2$, $\mathrm{Cl}_2$. Cho $0,35\mathrm{mol}$ $X$ vào bình thạch anh rồi đưa ra ánh sáng sau một thời gian thu được hỗn hợp $Y$ gồm $CH_3\mathrm{Cl}$, $\mathrm{HCl}$, $H_2$, $\mathrm{Cl}_2$. Hòa tan toàn bộ $Y$ vào nước dư thu được dung dịch $Z$ (bỏ qua phản ứng của $\mathrm{Cl}_2$ trong nước). Để trung hòa axit trong $Z$ thì cần $100\mathrm{ml}$ dung dịch chứa $\mathrm{NaOH} 0,1M$ và $\mathrm{Ba}(OH)_2$ $0,3$ M. Tính \% thể tích của khí hidroclorua trong Y.
\loigiai{
\Nhanmanh[\faComments]{Phân tích:}
\begin{itemize}
	\item  Mấu chốt bài toán ở sụ bảo toàn số mol khi trong mỗi phản úng:
		\[\begin{aligned}
			& \mathrm{CH}_4+\mathrm{Cl}_2 \xrightarrow[$\text{a/s}$] \mathrm{CH}_3 \mathrm{Cl}+\mathrm{HCl}&\quad &\left(\Delta\text{hệ số}=1+1-(1+1)=0 \right)  \\
			& \mathrm{H}_2+\mathrm{Cl}_2 \xrightarrow[$\text{a/s}$] 2 \mathrm{HCl} &\quad &\left(\Delta\text{hệ số}=1+1-2=0 \right)  \\
		\end{aligned}\]
	\item Trong phản úng giũa axit và bazơ thì chú ý phân tích mối liên hệ: $\mathrm{n}_{\mathrm{H}}(\mathrm{axit})=\mathrm{n}_{\mathrm{OH}}$ (bazơ $)$
\end{itemize}
\begin{center}{\indam{Lời giải:}}\end{center}
Các phương trình phản ứng:
\[\begin{aligned}
	& \mathrm{CH}_4+\mathrm{Cl}_2 \xrightarrow[$\text{a/s}$] \mathrm{CH}_3\mathrm{Cl}+\mathrm{HCl}&\quad&\text{(1)} \\
	& \mathrm{H}_2+\mathrm{Cl}_2 \xrightarrow[$\text{a/s}$] 2\mathrm{HCl}&\quad&\text{(2)}
\end{aligned}\]
Theo ptpư $(1,2)$ thấy số $\mathrm{mol}$ không đổi $\Rightarrow \mathrm{n}_{\mathrm{Y}}=\mathrm{n}_{\mathrm{X}}=0,35 \mathrm{~mol}$ Dung dịch $\mathrm{Z}$ chứa chất tan là $\mathrm{HCl}$.
\[\begin{aligned}
&2\mathrm{HCl}+\mathrm{Ba}(\mathrm{OH})_2 \xrightarrow \mathrm{BaCl}_2+2 \mathrm{H}_2 \mathrm{O}&\quad&\text{(3)}\\
&\mathrm{HCl}+\mathrm{NaOH} \xrightarrow \mathrm{NaCl}+\mathrm{H}_2\mathrm{O}&\quad&\text{(4)}
\end{aligned}\]
\\
Theo ptpu (3,4): $\mathrm{n}_{\mathrm{H}}($ axit $)=\mathrm{n}_{\mathrm{OH}}($ bazo $) \Rightarrow \mathrm{n}_{\mathrm{HCl}}=0,7.0,1=0,07 \mathrm{~mol}$
\\
Phần trăm thể tích khí $\mathrm{HCl}$ trong $\mathrm{Y}: \% \mathrm{~V}_{\mathrm{HCl}}=\frac{0,07}{0,35} \cdot 100 \%=20 \%$
}
\end{vd}
%%%%==============HetVidu10==============%%%
...
\begin{vd}
	...
	\loigiai{
		\Nhanmanh[\faComments]{Phân tích:} Câu cho tất cả các dữ kiện đều dạng tỉ lệ, đại lượng đề hỏi cũng tỉ lệ. Đây là cơ sở giúp ta khẳng định có thể sử dụng phương pháp tư chọn lương chất. Nếu bài toán mà tất cả đều là chất khí thì phân tích hệ số theo tăng giảm thể tích là phuoơng pháp khuyên dùng vì tiết kiệm đuợc thời gian. Để thấy được tính ưu việt của phương pháp phân tích, chúng ta hãy so sánh 2 cách giải sau đây.
		\begin{center}{\indam{Lời giải:}}\end{center}
		\begin{itemize}
			\item Cách 1: Phương pháp thông thưòng.
		Giả sử có 2 mol $N_2,7$ lít $N_2$ trong hỗn hợp $X$
		
		\[\begin{array}{ccccccc}
			&N_2&+&3 H_2& \xrightarrow[$t^\circ,\mathrm{xt}$][][1.5][0.5]& 2 NH_3&\\
			\text{Bđ:}&2&&7&&0& (mol)\\
			\text{Pu:}&x&&\xrightarrow[][][0.5] 3x&&2x&(mol)\\
			\text{Sau Pu:}&(2-x)&&(7-3x)&&2x&(mol)\\
		\end{array}\]
		...
		\item Cách 2: Phương pháp phân tích hệ số (tăng giảm thể tích).
...
\begin{vd}[Hỗn hợp nhiều chất có chung quy luật trong CTPT]
	...
	\loigiai{
				\Nhanmanh[\faComments]{Phân tích:}
				\begin{itemize}
					\item  E gồm các chất đều có phân tủ chúa 2 nguyên tủ $O$ và 2 liên kết pi. Do đó chúng có chung công thúc $\mathrm{C}_n \mathrm{H}_{2 n-2} \mathrm{O}_2$.
					\item  Từ số mol $\mathrm{CaCO}_3$ tính được số mol $\mathrm{CO}_2$ (số mol C)
					\item  Phân tích CTPT thấy $M=14 n+30 \Rightarrow \mathrm{m}_{\mathrm{E}}=14 \mathrm{n}_{\mathrm{C}}+30 \mathrm{n}_{\mathrm{E}} \Rightarrow \mathrm{n}_{\mathrm{E}}=$ ?; $\mathrm{n}_{\mathrm{H}_2 \mathrm{O}}=\mathrm{n}_{\mathrm{C}}-\mathrm{n}_{\mathrm{E}}$
				\end{itemize}
				\begin{center}{\indam{Lời giải:}}\end{center}
				...
				\[\begin{array}{ccccccccccc}
						& C_n H_{2n-2} O_2&+&(1,5n-1,5) O_2 &\xrightarrow[$t^\circ$][][][0.5] &\mathrm{nCO}_2&+&(n-1) H_2 O &\quad& \text{(1)}&\\
						& CO_2&+&\mathrm{Ca}(OH)_2 &\xrightarrow &\mathrm{CaCO}_3 \downarrow&+&H_2 O&\quad& \text{(2)}&\\
						&0{,}18&&&& \xleftarrow[][][0.5]0{,}18 &&&&& 
						\end{array}\]
...
\begin{vd}[Hỗn hợp nhiều chất có quy luật chung trong CTPT]
	...
	\loigiai{
				\Nhanmanh[\faComments]{Phân tích:} Nhìn hỗn hợp có lẽ các em học sinh phát ón. Tuy nhiên theo kinh nghiệm, hỗn hợp nhiều chất mà chỉ hỏi có một chất thì chắc chắn chất này kiểu nhu "cá biệt". Tách hỗn hợp thành 2 nhóm:
				\begin{itemize}
					\item Nhóm A: $C_6 H_6, C_3 H_8 O, C_2 H_4(CHO)_2, HCOOC_3 H_5, C_5 H_6 O$ (tổng chỉ số 12)
					\item Nhóm B: $C_3 H_8 O_3$ (tổng chỉ số 14)
				...
				\begin{itemize}
					\item Xử lí phản ứng với $KOH$
				\[\begin{aligned}
					& n_{KOH}=0,67 \mathrm{mol}; m_{\text{muuối}}=54,6(\mathrm{gam}) \\
					& \dfrac{138}{2}=69 < \dfrac{54,6}{0,67}=81,49 < 100 \Rightarrow \text{dung dịch có 2 muối} KHCO_3\ \text{và}\ K_2CO_3 \\
				...
				\\
				\Nhanmanh[\faBell]{Lưu ý:} Ngoài cách giải trên, các em học sinh có thể thục hiện ghép ẩn số, quy đổi hỗn hợp, bỏ bót chất. Hoặc tách ra 3 nhóm chất:
					\begin{itemize}
