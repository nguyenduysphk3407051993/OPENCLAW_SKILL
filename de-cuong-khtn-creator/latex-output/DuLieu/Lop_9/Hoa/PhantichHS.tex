...
\begin{vd}[Bài tập hỗn hợp chứa Fe và oxit sắt tác dụng với axit có tính oxi hóa mạnh]
	...
	\loigiai{
		...
		$$
		\begin{array}{l}
			\mathrm{Fe}_x O_y+(6 x-2 y) HNO_3 \to \mathrm{xFe}\left(NO_3\right)_3+(3 x-y) H_2 O+(3 x-2 y) NO_2 \uparrow\ (1)\left(n_{NO_2}=3 n_{\mathrm{Fe}}-2 n_O\right) * \\
			\mathrm{Cu}+4 HNO_3 \to \mathrm{Cu}\left(NO_3\right)_2+2 H_2 O+2 NO_2 \uparrow\ (2) \\
			\mathrm{Mg}+4 HNO_3 \to \mathrm{Mg}\left(NO_3\right)_2+2 H_2 O+2 NO_2 \uparrow\ (3)
		\end{array}
		$$
...
%%%%==============HetVidu5==============%%%
%%%==============Vidu6==============%%%
\begin{vd}(Phân tích các mối liên hệ trong phân tủ chất béo)
	...
	\loigiai{
		...
		%		$$
		%		\begin{gathered}
			%			C_{17} H_{35-2 k} \mathrm{COONa}+\mathrm{kBr}_2 \to C_{17} H_{35-2 k} \mathrm{Br}_{2 k} \mathrm{COONa} \\
...
\begin{vd}[Câu hỗn hợp kim loại kiềm,kiềm thổ và oxit của chúng]
	...
	\loigiai{
		...
		%	
		%	$$
		%	\mathrm{Na}_2 O_x+(2-x) H_2 O \to 2 \mathrm{NaOH}+(1-x) H_2 \uparrow \quad \text{(1)}\left(H \widehat{̣} \text{số} H_2=1-x=1 / 2 \text{chỉ số} \mathrm{Na}-\text{chỉ số} O\right)
		%	$$
		%	
		%	$$
		%	\mathrm{BaO}_y+(2-y) H_2 O \to \mathrm{Ba}(OH)_2+(1-y) H_2 \uparrow \quad \text{(2)}\left(H \widehat{̣} \text{số} H_2=1-y=\text{chỉ số} \mathrm{Ba}-\text{chỉ số} O\right)
		%	$$
		%	
		...
		%	$$
		%	\begin{array}{ll}
			%		\mathrm{Na}_2 O_x+(2-x) H_2 O \to 2 \mathrm{NaOH}+(1-x) H_2 \uparrow & \text{(1)}\left(n_{H_2}=\dfrac{1}{2} n_{\mathrm{Na}}-n_O\right) \\
			%		\mathrm{BaO}_y+(2-y) H_2 O \to \mathrm{Ba}(OH)_2+(1-y) H_2 \uparrow & \text{(2)}\left(n_{H_2}=n_{\mathrm{Ba}}-n_O\right)
			%	\end{array}
		%	$$
		%	
		...
		%	$$
		%	\begin{array}{ll}
			%		CO_2+\mathrm{Ba}(OH)_2 \to \mathrm{BaCO}_3 \downarrow+H_2 O & \text{(3)}\left(n_{OH}-n_{CO_2}=n_{\mathrm{BaCO}_3}\right) \\
			%		2 CO_2+\mathrm{Ba}(OH)_2 \to \mathrm{Ba}\left(HCO_3\right)_2 & \text{(4)}\left(n_{OH}-n_{CO_2}=0\right) \\
			%		CO_2+\mathrm{NaOH} \to \mathrm{NaHCO}_3 & \text{(5)}\left(n_{OH}-n_{CO_2}=0\right)
			%	\end{array}
		%	$$
		%	
...
\begin{vd}
	...
	\loigiai{
		%		Phân tích: Đây là một bài tập không khó nhưng khá hay. Dễ thấy hỗn hợp truớc và sau phản úng đều giũ nguyên các nguyên tố ban đầu nên không lượng không đổi ($\left.m_X=m_{CH_4}=8\mathrm{gam}\right)$.
		%		Phân tích hệ số phản úng: $2CH_4 \xrightarrow{t^0} C_2 H_2+3H_2$ thấy $\Delta$ hệ số $=1+3-2=2=$ hệ số $CH_4$. Nhu vậy trong phản úng này số mol khi tăng lên đúng bằng số mol khí $CH_4$ phản úng.
		...
		%		\begin{gathered}
			...
			%			2CH_4 \xrightarrow{t^0} C_2 H_2+3H_2 \\
			%			0,5 \mathrm{h} \to \quad 0,25 \mathrm{h} \quad 0,75 \mathrm{h}(\mathrm{mol})
			%		\end{gathered}
		%		$$
...
\begin{vd}
	...
	\loigiai{
		%	Phân tích: Mấu chốt bài toán ở các đặc điểm sau:-Xem xét TGKL kim loại ta thấy: $\mathrm{Zn} \to \mathrm{Cu}$ (khối luợng giảm); Fe $\to \mathrm{Cu}$ (khối luợng tăng).
		%	Nếu Fe chưa phản úng thì khối lượng KL giảm. Theo đề khối lượng KL lại tăng $\Rightarrow$ chúng tỏ Fe đã phản úng, Zn đã hết.
		...
		%	$$
		%	\begin{aligned}
			%		& \mathrm{Zn}+\mathrm{Cu}\left(NO_3\right)_2 \to \mathrm{Zn}\left(NO_3\right)_2+\mathrm{Cu} \downarrow \\
			%		& x \to x \\
			%		& \mathrm{Fe}+\mathrm{Cu}\left(NO_3\right)_2 \to \mathrm{Fe}\left(NO_3\right)_2+\mathrm{Cu} \downarrow \\
			%		& y \to y \\
			...
			%		& \mathrm{Zn}(OH)_2 \xrightarrow{t^0} \mathrm{Kn} \text{tăng} \Delta \mathrm{me}=8 y(g) \\
			%		& 2 OH)_2 \downarrow+2 \mathrm{NaNO}_3 \\
			%		& 2 \mathrm{Fe}(OH)_2+1 / 2 O_2 \xrightarrow{t^0} \mathrm{Fe}_2 O_3+2 H_2 O
			%	\end{aligned}
		%	$$
		%	
...
\begin{vd}
	...
	\loigiai{
		%		Phân tích+Mấu chốt bài toán ở sụ̣ bảo toàn số mol khí trong mỗi phản úng:
		%		$$
		%		\begin{array}{ll}
			...
			%		\end{array}
		%		$$+Trong phản úng giũa axit và bazơ thì chú ý phân tích mối liên hệ: $n_H(\mathrm{axit})=n_{OH}($ bazơ $)$
		%		Các phương trình phản ứng:
		%		$$
		%		\begin{align*}
			%			& CH_4+\mathrm{Cl}_2 \xrightarrow{a / s} CH CH_3 \mathrm{Cl}+\mathrm{HCl} \tag{1}\\
			%			& H_2+\mathrm{Cl}_2 \xrightarrow{a / s} 2\mathrm{HCl} \tag{2}
			%		\end{align*}
		%		$$
		...
		%		$$
		%		\begin{equation*}
			%			2 \mathrm{HCl}+\mathrm{Ba}(OH)_2 \to \mathrm{BaCl}_2+2 H_2 O \tag{3}
			%		\end{equation*}
		%		$$
		%		$$
		%		\begin{equation*}
			%			\mathrm{HCl}+\mathrm{NaOH} \to \mathrm{NaCl}+H_2 O \tag{4}
			%		\end{equation*}
		%		$$
...
\begin{vd}
	...
	\loigiai{
		...
		%		$$
		%		N_2+3 H_2 \xrightarrow{t^0, \mathrm{xt}} 2 NH_3
		%		$$
		%		Bđ: 27
		%		$$
		%		0 \text{(mol)}
		...
		%		\begin{array}{lr}
			%			N_2+3 H_2 \xrightarrow{t^0, \mathrm{xt}} 2 NH_3 \\
			%			2 \mathrm{h} \to 6 \mathrm{h} & 4 \mathrm{h}(\mathrm{mol})
			%		\end{array}
		%		$$
		%		
...
\begin{vd}[Hỗn hợp nhiều chất có chung quy luật trong CTPT]
	...
	\loigiai{
		...
		%		$$
		%		\begin{align*}
			%			& C_n H_{2n-2} O_2+(1,5n-1,5) O_2 \xrightarrow{t^0} \mathrm{nCO}_2+(n-1) H_2 O\tag{1}\\
			%			& CO_2+\mathrm{Ca}(OH)_2 \to \mathrm{CaCO}_3 \downarrow+H_2 O\tag{2}\\
			%			& 0,18\end{align*}
		%		$$
...
\begin{vd}[Hỗn hợp nhiều chất có quy luật chung trong CTPT]
	...
	\loigiai{
		...
		%		$$
		%		C_x H_y O_z+(x+0,25 y-0,5 z) O_2 \xrightarrow{t^0} \mathrm{xCO}_2+0,5 \mathrm{yH}_2 O
		%		$$
		%		$\checkmark$ Xử lí phản ứng với $KOH$
		%		$$
		%		\begin{aligned}
			%				& n_{KOH}=0,67 \mathrm{mol}; m_{\text{muб́i}}=54,6(\mathrm{gam}) \\
			%				& \dfrac{138}{2}=69 < \dfrac{54,6}{0,67}=81,49 < 100 \to \text{dung dịch có} 2 \text{muối} KHCO_3 \text{và} K_2 CO_3 \\
			%				& CO_2+2 KOH \to K_2 CO_3+H_2 O \\
			%				& CO_2+KOH \to KHCO_3
			%			\end{aligned}
		%		$$
...
\begin{vd} (Phân tich theo tăng giảm số mol)
	...
	\loigiai{
		...
		%		$$
		%		\begin{aligned}
			%				& \mathrm{CaC}_2+2 \mathrm{HCl} \to \mathrm{CaCl}_2+C_2 H_2 \uparrow \\
			%				& \mathrm{Ca}+2 \mathrm{HCl} \to \mathrm{CaCl}_2+H_2 \uparrow
			%			\end{aligned}
		%		$$
		%		$$
		%		\begin{aligned}
		...
		%		$$
		%		\begin{align*}
			%				& \mathrm{Al}_4 C_3+12\mathrm{HCl} \to 4\mathrm{AlCl}_3+3CH_4 \uparrow \tag{3}\\
			%				& \mathrm{FeS}+2\mathrm{HCl} \to \mathrm{FeCl}_2+H_2 \mathrm{S} \uparrow \\
...
\begin{vd}[Phân tích độ lệch giũa số mol hỗn hợp và số mol nguyên tố]
	...
	\loigiai{
		%		Phân tích: Hỗn hợp $X$ có 5 chất mà đề bài chỉ yêu cầu tính $\%$ của $\mathrm{Fe}_3 O_4$. Điều này cho thấy $\mathrm{Fe}_3 O_4$ có có sụ̣ khác biệt về cấu tạo phân tủ. Rất dễ nhìn thấy trong $\mathrm{Fe}_3 O_4$ có số mol kim loại gấp 3 lần số mol hợp chất (hay $n_{KL}-n_{\mathrm{hc}}=2n_{\mathrm{Fe}_3 O_4}$), các chất còn lại có hiệu này bằng 0 Mặt khác: Hỗn hợp rắn $Y$ gồm $\mathrm{CuO}, \mathrm{Fe}_2 O_3$ đều có $M=80$ lần số mol $KL \Rightarrow$ tổng số mol $KL=\dfrac{m_Y}{80}$.
		%		Các phương trình hóa học:
		%		$$
		%		\begin{aligned}
			%			& 2 \mathrm{FeO}+1 / 2 O_2 \xrightarrow{t^0} \mathrm{Fe}_2 O_3 \\
			%			& 2 \mathrm{Fe}_3 O_4+1 / 2 O_2 \xrightarrow{t^0} 3 \mathrm{Fe}_2 O_3 \\
			%			& 2 \mathrm{Fe}(OH)_2+1 / 2 O_2 \xrightarrow{t^0} \mathrm{Fe}_2 O_3+2 H_2 O \\
			%			& \mathrm{Cu}(OH)_2 \xrightarrow{t^0} \mathrm{CuO}+H_2 O
			%		\end{aligned}
		%		$$
...
%%%==============Vidu16==============%%%
\begin{vd}[Phân tích phản úng giũa hỗn hợp ($\mathrm{NaHCO}_3$ và $\mathrm{Na}_2 CO_3$) tác dụng dung dịch axit]
	...
	\loigiai{
		%		Phân tích: Thí nghiệm của $X$ với nuoớc thì đã quá quen thuộc. Ở đây chúng ta phân tích thí nghiệm của T (chía $\mathrm{Na}_2 CO_3$ và $\mathrm{NaHCO}_3$) tác dụng vói dung dịch $\mathrm{HCl}$:+Lượng HCl không đổi mà thể tích $CO_2$ khác nhau $\Rightarrow$ chúng tỏ HCl thiếu (muối trong T chưa hết)+Cho từ từ T vào HCl phản úng song song nên tỉ lệ số mol muối phản úng bằng tỉ lệ số mol ban đầu.
		%		$$
		%		\begin{aligned}
			%			& \mathrm{Na}_2 CO_3+2 \mathrm{HCl} \to 2 \mathrm{NaCl}+H_2 O+CO_2 \uparrow\left(n_{\mathrm{HCl}}: n_{CO_2}=2\right) \\
			%			& \mathrm{NaHCO}_3+\mathrm{HCl} \to \mathrm{NaCl}+H_2 O+CO_2 \uparrow \quad\left(n_{\mathrm{HCl}}: n_{CO_2}=1\right)
			%		\end{aligned}
		%		$$
		%		$\Rightarrow$ Sủ dụng $QTĐ C$ ta tìm được tỉ lệ số mol $\mathrm{NaHCO}_3$ và $\mathrm{Na}_2 CO_3$+Cho từ từ HCl vào T phản úng nối tiếp, có sinh khí nên $\mathrm{Na}_2 CO_3$ hết.
		%		$$
		%		\begin{aligned}
			%			& \mathrm{Na}_2 CO_3+\mathrm{HCl} \to \mathrm{NaCl}+\mathrm{NaHCO}\left(n_{\mathrm{HCl}}-n_{CO_2}=n_{\mathrm{Na}_2 CO_3}\right) \\
			%			& \mathrm{NaHCO}_3+\mathrm{HCl} \to \mathrm{NaCl}+H_2 O+CO_2 \uparrow \quad\left(n_{\mathrm{HCl}}-n_{CO_2}=0\right)
			%		\end{aligned}
		%		$$
		...
		%		$$
		%		\begin{align*}
			%			& \mathrm{Na}_2 O+H_2 O\to 2\mathrm{NaOH} \tag{1}\\
			%			& \mathrm{BaO}+H_2 O\to \mathrm{Ba}(OH)_2 \tag{2}\\
			%			& 2\mathrm{Na}+2H_2 O\to 2\mathrm{NaOH}+H_2 \uparrow \tag{3}\\
			%			& \mathrm{Ba}+2H_2 O\to \mathrm{Ba}(OH)_2+H_2 \uparrow \tag{4}
			%		\end{align*}
		%		$$
		%		$\checkmark$ Phản ứng của $Y$ với $CO_2: n_{CO_2}=0,32\mathrm{mol}$
		%		$$
		%		\begin{align*}
			%			& CO_2+\mathrm{Ba}(OH)_2 \to \mathrm{BaCO}_3 \downarrow+H_2 O\tag{5}\\
			%			& CO_2+2\mathrm{NaOH} \to \mathrm{Na}_2 CO_3+H_2 O\tag{6}\\
			%			& CO_2+\mathrm{NaOH} \to \mathrm{NaHCO}_3 \tag{7}
			%		\end{align*}
		%		$$
		...
		%		$$
		%		\begin{array}{ll}
			%			\mathrm{Na}_2 CO_3+2 \mathrm{HCl} \to 2 \mathrm{NaCl}+H_2 O+CO_2 \uparrow & \text{(8)}\left(n_{\mathrm{HCl}}: n_{CO_2}=2\right) \\
			%			\mathrm{NaHCO}_3+\mathrm{HCl} \to \mathrm{NaCl}+H_2 O+CO_2 \uparrow & \text{(9)}\left(n_{\mathrm{HCl}}: n_{CO_2}=1\right)
			%		\end{array}
		%		$$
		%		Theo đề: $n_{\mathrm{HCl}}: n_{CO_2}=0,12: 0,075=1,6\to \dfrac{n_{\mathrm{NaHCO}_3}}{n_{\mathrm{Na}_2 CO_3}}=\dfrac{2-1,6}{1,6-1}=\dfrac{2}{3}$
		%		Phần 2: Phản ứng xảy ra theo thứ tự
		%		$$
		%		\begin{align*}
			%			& \mathrm{Na}_2 CO_3+\mathrm{HCl} \to \mathrm{NaHCO}_3+\mathrm{NaCl} \tag{10}\\
			%			& \mathrm{NaHCO}_3+\mathrm{HCl} \to \mathrm{NaCl}+H_2 O+CO_2 \uparrow \tag{11}
			%		\end{align*}
		%		$$
		%		Theo pt phản ứng (10,11): $n_{\mathrm{Na}_2 CO_3}=n_{\mathrm{HCl}}-n_{CO_2}=0,12-0,06=0,06\mathrm{mol}$
		%		$$
		%		\to n_{\mathrm{NaHCO}_3}=\dfrac{0,06.2}{3}=0,04 \mathrm{mol}
		%		$$
		%		BT mol cabon $\to n_{\mathrm{BaCO}_3}=0,32-0,08-0,12=0,12\mathrm{mol}$
		%		$\to Y: 0,32\mathrm{mol} \mathrm{NaOH}, 0,12\mathrm{mol} \mathrm{Ba}(OH)_2$
		%		BT mol $H\to n_{\left.H_2 O\text{(phản úng} 1,2,3,4\right)}=(0,32+0,12.2+0,15.2): 2=0,43\mathrm{mol}$
		%		$BTKL \to m_X+m_{H_2 O}=m_{\mathrm{Ba}(OH)_2}+m_{\mathrm{NaOH}}+m_{H_2}$
...
\begin{vd}[Sử dụng tỉ lệ số mol $OH$ $:$ $CO_2$ làm hệ số trong phản ứng $CO_2$ tác dụng hỗn hợp kiềm]
	Dẫn từ từ 4,48 lít $CO_2$ (đktc) vào $200\mathrm{ml}$ dung dịch $X$ chứa $\mathrm{NaOH} 0,5M$ và $\mathrm{Ba}(OH)_2 0,3M$ đến khi phản ứng hoàn toàn thu được $m$ gam kết tủa và gam dung dịch $Y$. Tính giá trị của $m$ và nồng độ $\%$ chất tan trong Y. Giả sử dung dịch $X$ có khối lượng riêng $\mathscr{D}=1,25\mathrm{g} / \mathrm{ml}$.
	\loigiai{
		...
		$$
		\begin{aligned}
			& 10 CO_2+11 XOH \to X_2 CO_3+9 XHCO_3+H_2 O \\
			& 0,2 \to \quad 0,02 \quad 0,18(\mathrm{mol})
		\end{aligned}
		$$
