...
\newcommand{\phan}[2][\mycolor]{%
	...
	\par\noindent\begin{tikzpicture}[declare function={d=0.2cm;r=0.6cm;}]
		\path (0,0) node[font=\fontfamily{qag}\selectfont\bfseries\color{white},inner sep=2pt,outer sep =2pt](PHAN){\Roman{phan}.\,#2};
	...
	\end{tikzpicture}%	
	\vspace{6pt}
	\addcontentsline{toc}{subsubsection}{\Roman{phan}.\ #2}
}
\def\muctieu{}
%%%=============Hộp mục tiêu==========%%%
\newtcolorbox{Muctieu}{
	enhanced jigsaw,
	before skip =6pt, after skip= 6pt,left skip= .3cm,
	grow to right by =2.5cm,
	left=0.2mm,right=2.5cm,
	boxsep=10pt,
	boxrule=0mm,
	arc=6pt,
	arc is angular,
...
%		\draw[\mycolor!90!black,rounded corners=2pt,thick] (frame.north west)--(frame.south west)-- (frame.south east)--(frame.north east);
	},
	overlay middle and last ={
		\path 
		([shift={(0.6cm,-0.1cm)}]frame.north west) node[fill=violet!80,inner sep=0pt,outer sep =-1pt,shape=circle] (note)
...
\newcommand{\NoteIcon}{%
	\includegraphics[width=0.85cm]{./Images/Icons/Icon3d_22.png}
}

\newcommand{\MascotIcon}{%
	\includegraphics[width=1cm]{./Images/Icons/PPG.png}
}

\pgfkeys{/rememberbox/.is family,/rememberbox,
	title/.initial={Kiến thức cần nhớ},
	icon/.initial={\NoteIcon},
	mascot/.initial={\MascotIcon},
	colback/.initial=white,
	colframe/.initial=\mycolor!30!black,
	titleback/.initial=\mycolor!50,
	titleframe/.initial=\mycolor!50!black,
	titlecolor/.initial=black,
	arc/.initial=13pt,
	boxrule/.initial=0.55pt,
	dash pattern/.initial={on 3pt off 2.2pt},
	title font/.initial={\fontfamily{qag}\bfseries\Large},
	body padding/.initial=3mm
}

\NewDocumentEnvironment{rememberbox}{O{} +b}{%
	\pgfkeys{/rememberbox,#1}%
	\edef\rememberdash{\pgfkeysvalueof{/rememberbox/dash pattern}}%
	\begin{tcolorbox}[
		enhanced,
		breakable,
		empty,
		before skip=3mm,
		after skip=3mm,
		colback=\pgfkeysvalueof{/rememberbox/colback},
		colframe=\pgfkeysvalueof{/rememberbox/colframe},
		boxrule=\pgfkeysvalueof{/rememberbox/boxrule},
		arc=\pgfkeysvalueof{/rememberbox/arc},
		outer arc=\pgfkeysvalueof{/rememberbox/arc},
		boxsep=0pt,
		left=\pgfkeysvalueof{/rememberbox/body padding},
		right=\pgfkeysvalueof{/rememberbox/body padding},
		top=13mm,
		bottom=8mm,
		pad at break* =3mm,
		overlay unbroken and first={%
			\path[fill=\mycolor!5, draw=\mycolor!80, rounded corners=3mm]
			(frame.south west) -- (frame.north west) -- (frame.north east)
			[sharp corners] -- (frame.south east) -- cycle;
			\node[anchor=north,draw=\pgfkeysvalueof{/rememberbox/titleframe},fill=\pgfkeysvalueof{/rememberbox/titleback},rounded corners=5pt,line width=0.8pt,inner xsep=10pt,inner ysep=4pt,text=\pgfkeysvalueof{/rememberbox/titlecolor},minimum width={min(0.44\linewidth,10cm)},align=center,minimum height=1.2cm] (rtitle) at ([yshift=6mm]frame.north) {\pgfkeysvalueof{/rememberbox/title font}\pgfkeysvalueof{/rememberbox/title}};
			\node[anchor=east,inner sep=0pt] at ([xshift=11mm,yshift=0mm]rtitle.west) {\pgfkeysvalueof{/rememberbox/icon}};
			\node[anchor=west,inner sep=0pt] at ([xshift=-10mm,yshift=0mm]rtitle.east) {\pgfkeysvalueof{/rememberbox/mascot}};
		},
		overlay middle ={%
			\path[fill=\mycolor!5, draw=\mycolor!80] (frame.north west)--(frame.north east)--(frame.south east)--(frame.south west)--cycle;
		},
		overlay last ={%
		\path[fill=\mycolor!5, draw=\mycolor!80]
		(frame.south west) -- (frame.north west) -- (frame.north east)
		[rounded corners=3mm] -- (frame.south east) -- cycle;
		}
		]%
		#2%
}{\end{tcolorbox}}
\NewDocumentEnvironment{kienthuccannho}{O{} +b}{\begin{rememberbox}[#1]#2\end{rememberbox}}{}
