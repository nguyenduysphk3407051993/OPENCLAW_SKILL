...
# \documentclass[FileMain.tex]{subfiles} -> bắt lấy tên file master
MASTER_RE = re.compile(r"\\documentclass\s*\[([^\]]+)\]\s*\{subfiles\}")
...
def kiem_tra_master(goc):
    """Mọi file subfiles phải trỏ tới một file master CÓ THẬT, đúng HOA/thường.

    Đây là lỗi hay gặp nhất khi bê skill sang máy khác: template ghi
    `\\documentclass[FileMain.tex]{subfiles}` nhưng trong thư mục build file
    master lại tên `Main.tex`. Trên Windows lẫn Linux đều vỡ ngay từ dòng đầu,
    và thông báo lỗi của LaTeX thì không hề nhắc tới tên file bị thiếu.
    """
    thuc = quet_file_thuc(goc)
    thuc_low = {p.lower(): p for p in thuc}
    lech, thieu, n = [], [], 0
    for rel in sorted(p for p in thuc if p.lower().endswith(".tex")):
        try:
            with open(os.path.join(goc, rel), "r", encoding="utf-8",
                      errors="ignore") as f:
                noi_dung = f.read()
            ...
            continue
        m = MASTER_RE.search(noi_dung)
        if not m:
            continue
        n += 1
        master = _norm(m.group(1).strip())
        ung_vien = [master, _norm(master + ".tex")]
        if any(c in thuc for c in ung_vien):
            continue
        khop = next((thuc_low[c.lower()] for c in ung_vien
                     if c.lower() in thuc_low), None)
        if khop:
            lech.append((rel, m.group(1).strip(), khop))
        else:
            thieu.append((rel, m.group(1).strip()))
    return n, lech, thieu
...
def main():
    ...
    # 2. File master của subfiles
    n_mas, lech_mas, thieu_mas = kiem_tra_master(goc)
    print(f"[2] File subfiles khai bao master: {n_mas}")
    if thieu_mas:
        loi += len(thieu_mas)
        print(f"    [LOI] {len(thieu_mas)} file tro toi master KHONG TON TAI - "
              f"vo ngay dong dau, moi he dieu hanh:")
        for src, ref in thieu_mas[:20]:
            print(f"      {src}: \\documentclass[{ref}]{{subfiles}}")
    if lech_mas:
        loi += len(lech_mas)
        print(f"    [LOI] {len(lech_mas)} file tro toi master SAI HOA/THUONG:")
        for src, ref, that in lech_mas[:20]:
            print(f"      {src}: [{ref}]  ->  file that: {that}")
    if n_mas and not lech_mas and not thieu_mas:
        print("    [OK] moi file subfiles deu tro dung master")

    # 3. Tên file/thư mục có dấu hoặc khoảng trắng
    xau = [p for p in quet_file_thuc(goc)
           if any(ord(c) > 127 for c in p) or " " in p]
    print(f"[3] Ten file co dau/khoang trang: {len(xau)}")
    if xau:
    ...
    print(f"[5] Goi LaTeX dac biet dang dung: {', '.join(can) or '(khong)'}")
    if can:
